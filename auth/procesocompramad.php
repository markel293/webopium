...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$resultado_correo || mysqli_num_rows($resultado_correo) === 0) {
        header("Location: ../OpiumMadrid/entradasmad.php?dia=$dia&error=El correo introducido no está registrado, por favor regístrate antes de comprar cualquier entrada.#formulario");
        exit;
    }
    ...
    if (!$resultado_entrada || mysqli_num_rows($resultado_entrada) === 0) {
        header("Location: ../OpiumMadrid/entradasmad.php?dia=$dia&error=Lote de entrada no encontrado.#formulario");
        exit;
    }
    ...
    // Redirigir con éxito
    header("Location: ../OpiumMadrid/entradasmad.php?dia=$dia&ok=Compra realizada con éxito.#formulario");
    exit;
